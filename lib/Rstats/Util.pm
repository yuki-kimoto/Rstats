...
sub parse_index {
  ...
  if (ref $_indexs[0] && $_indexs[0]->is_array && $_indexs[0]->is_logical && @{$_indexs[0]->dim->elements} > 1) {
    ...
    for (my $i = 0; $i < @$a1_dim; $i++) {
      ...
      my $index = Rstats::ArrayAPI::to_array($_index);
      ...
      if (!@{$index->values}) {
        ...
        $index = Rstats::ArrayAPI::array($index_values_new);
      }
        ...
        if ($a1->is_vector) {
          ...
          $indexs[$i] = Rstats::ArrayAPI::array($index_new_values);
        }
        elsif ($a1->is_matrix) {
        ...
        $index = Rstats::ArrayAPI::array($index_value_new);
      }
