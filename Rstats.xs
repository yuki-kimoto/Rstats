  ...
  PPCODE:
{
  ...
  if (items > 2) {
    sv_x_out = Rstats::Func::c_double(sv_r, sv_values, ST(2));
  }
  else {
    sv_x_out = Rstats::Func::c_double(sv_r, sv_values, &PL_sv_undef);
  }
  ...
  PPCODE:
{
  ...
  SV* sv_result;
  if (items > 2) {
    sv_result = Rstats::Func::array(sv_r, ST(1), ST(2));
  }
  else {
    sv_result = Rstats::Func::array(sv_r, ST(1), &PL_sv_undef);
  }
  XPUSHs(sv_result);
  XSRETURN(1);
}
