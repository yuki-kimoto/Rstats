...
# get
{
  # get - drop
  {
    my $x1 = $r->factor($r->c_("a1", "a2", "a3", "a1", "a2", "a3"));
    my $x2 = $x1->get($r->c_(4, 6), {drop => $r->TRUE});
    ok(r->is->factor($x2));
  ...
  # get - ordered
  {
    my $x1 = $r->ordered($r->c_("a1", "a2", "a3", "a1", "a2", "a3"));
    my $x2 = $x1->get($r->c_(4, 6));
    ok(r->is->factor($x2));
    ok(r->is->ordered($x2));
    is_deeply($x2->values, [1, 3]);
...
# nlevels
{
  # nlevels - set values
  {
    my $x1 = $r->factor($r->c_("a1", "a2", "a1", "a2"));
...
# interaction
{
  ...
  {
    my $x1 = $r->factor($r->c_("a1", "a2", "a3"));
    my $x2 = $r->factor($r->c_("b1", "b2"));
    my $x3 = $r->factor($r->c_("c1"));
...
# gl
{
  ...
  # gl - labels
  {
    my $x1 = r->gl(3, 3, {labels => $r->c_("a", "b", "c")});
    ok(r->is->factor($x1));
    is_deeply($x1->values, [1, 1, 1, 2, 2, 2, 3, 3, 3]);
...
# ordered
{
  # ordered - basic
  {
    my $x1 = $r->ordered($r->c_("a", "b", "c", "a", "b", "c"));
    ok(r->is->ordered($x1));
    ok(r->is->integer($x1));
    ok(r->is->factor($x1));
  ...
  # ordered - option
  {
    my $x1 = $r->ordered($r->c_("a", "b", "c", "a", "b", "c"), {levels => $r->c_("a", "b", "c")});
    ok(r->is->ordered($x1));
    ok(r->is->integer($x1));
    ok(r->is->factor($x1));
    is_deeply($x1->values, [1, 2, 3, 1, 2 ,3]);
...
# factor
{
  # factor - one element
  {
    my $x1 = $r->factor($r->c_("a"));
    is_deeply($x1->values, [1]);
    is_deeply(r->levels($x1)->values, ["a"]);
  }
  
  # factor - as.numeri$r->c_(levels(f))[f] 
  {
    my $x1 = $r->factor($r->c_(2, 3, 4, 2, 3, 4));
  ...
  # factor - labels
  {
    my $x1 = $r->factor($r->c_("a", "b", "c", "a", "b", "c"));
    my $x2 = r->labels($x1);
  ...
  # factor - ordered
  {
    my $x1 = $r->factor($r->c_("a", "b", "c", "a", "b", "c"), {ordered => $r->TRUE});
    ok(r->is->ordered($x1));
  }
  ...
  # factor - exclude
  {
    my $x1 = $r->factor($r->c_("a", "b", "c", "a", "b", "c"), {exclude => "c"});
    is_deeply($x1->values, [1, 2, undef, 1, 2, undef]);
  }
  
  # factor - labels
  {
    my $x1 = $r->factor($r->c_("a", "b", "c", "a", "b", "c"), {levels => $r->c_("a", "b", "c"), labels => $r->c_(1, 2, 3)});
    my $expected = <<'EOS';
[1] 1 2 3 1 2 3
Levels: 1 2 3
EOS
  ...
  {
    my $x1 = $r->factor($r->c_("a", "b", "c", "a", "b", "c"), {levels => $r->c_("a", "b", "c"), labels => "a"});
    my $expected = <<'EOS';
[1] a1 a2 a3 a1 a2 a3
Levels: a1 a2 a3
EOS
  ...
  # factor - to_string
  {
    my $x1 = $r->factor($r->c_("a", "b", "c", "a", "b", "c"), {levels => $r->c_("a", "b")});
    my $expected = <<'EOS';
[1] a b <NA> a b <NA>
Levels: a b
EOS
