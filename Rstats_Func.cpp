...
namespace Rstats {
  namespace Func {
    ...
    Rstats::Size get_length (SV* sv_r, SV* sv_x1) {
    ...
    SV* as_character(SV* sv_r, SV* sv_x1) {
      ...
      if (strEQ(type, "character")) {
        ...
        if (to_bool(sv_r, Rstats::Func::is_factor(sv_r, sv_x1))) {
          ...
          Rstats::Size x1_values_length = Rstats::pl_av_len(sv_x1_values);
          for (Rstats::Size i = 0; i < x1_values_length; i++) {
    ...
    SV* create_sv_value(SV* sv_r, SV* sv_x1, Rstats::Size pos) {
      ...
      if (strEQ(type, "character")) {
        ...
        if (v1->exists_na_position(pos)) {
          ...
          Rstats::Size value = v1->get_value<Rstats::Integer>(pos);
    ...
    SV* upgrade_length_avrv(SV* sv_r, SV* sv_xs) {
      ...
      Rstats::Size xs_length = Rstats::pl_av_len(sv_xs);
      Rstats::Size max_length = 0;
    ...
    void upgrade_length(SV* sv_r, Rstats::Size num, ...) {
      va_list args;
    ...
    void upgrade_type(SV* sv_r, Rstats::Size num, ...) {
      va_list args;
    ...
    SV* c(SV* sv_r, SV* sv_elements) {
      ...
      Rstats::Size element_length = Rstats::pl_av_len(sv_elements);
      // Check type and length
      SV* sv_type_h = Rstats::pl_new_hvrv();
      Rstats::Size length = 0;
      for (Rstats::Size i = 0; i < element_length; i++) {
        char* type;
      ...
      for (Rstats::Size i = 0; i < element_length; i++) {
        ...
        if (to_bool(sv_r, Rstats::Func::is_vector(sv_r, sv_element)) || to_bool(sv_r, Rstats::Func::is_array(sv_r, sv_element))) {
          ...
          for (Rstats::Size k = 0; k < Rstats::Func::get_length(sv_r, sv_x_tmp); k++) {
    ...
    SV* array_with_opt(SV* sv_r, SV* sv_args_h) {
      ...
      if (SvUV(sv_x1_length) == dim_product) {
        ...
        Rstats::Size elements_tmp2_length = Rstats::pl_av_len(sv_elements_tmp2);
        for (Rstats::Size i = 0; i < repeat_count; i++) {
          for (Rstats::Size k = 0; k < elements_tmp_length; k++) {
    ...
    SV* upgrade_type_avrv(SV* sv_r, SV* sv_xs) {
      ...
      SV* sv_new_xs = Rstats::pl_new_avrv();;
      Rstats::Size type_length = Rstats::pl_hv_key_count(sv_type_h);
    ...
    SV* dim(SV* sv_r, SV* sv_x1, SV* sv_x_dim) {
      ...
      Rstats::Size x_dim_values_length = Rstats::pl_av_len(sv_x_dim_values);
    ...
    SV* length_value(SV* sv_r, SV* sv_x1) {
      ...
      if (strEQ(SvPV_nolen(Rstats::pl_hv_fetch(sv_x1, "object_type")), "NULL")) {
        ...
        Rstats::Size length = Rstats::pl_av_len(sv_list);
        sv_length = Rstats::pl_new_sv_iv(length);
      }
    ...
    SV* c_character(SV* sv_r, SV* sv_values) {
      ...
      Rstats::Size length = Rstats::pl_av_len(sv_values);
      ...
      SV* sv_x1 = Rstats::Func::new_vector<Rstats::Character>(sv_r);
    ...
    SV* c_double(SV* sv_r, SV* sv_values) {
      ...
      SV* sv_x1 = Rstats::Func::new_vector<Rstats::Double>(sv_r);
      set_vector(sv_r, sv_x1, v1);
    ...
    SV* c_complex(SV* sv_r, SV* sv_values) {
      ...
      SV* sv_x1 = Rstats::Func::new_vector<Rstats::Complex>(sv_r);
      set_vector(sv_r, sv_x1, v1);
    ...
    SV* c_integer(SV* sv_r, SV* sv_values) {
      ...
      SV* sv_x1 = Rstats::Func::new_vector<Rstats::Integer>(sv_r);
      set_vector(sv_r, sv_x1, v1);
    ...
    SV* c_logical(SV* sv_r, SV* sv_values) {
      ...
      SV* sv_x1 = Rstats::Func::new_vector<Rstats::Logical>(sv_r);
      set_vector(sv_r, sv_x1, v1);
    ...
    SV* to_c(SV* sv_r, SV* sv_x) {
      ...
      Rstats::Logical is_container = sv_isobject(sv_x) && sv_derived_from(sv_x, "Rstats::Object");
    ...
    SV* is_na(SV* sv_r, SV* sv_x1) {
      ...
      sv_x1 = Rstats::Func::to_c(sv_r, sv_x1);
      Rstats::Vector* v1 = Rstats::Func::get_vector(sv_r, sv_x1);
      Rstats::Size x1_length = Rstats::Func::get_length(sv_r, sv_x1);
      Rstats::Vector* v2 = Rstats::VectorFunc::new_vector<Rstats::Logical>(x1_length);
      
      for (Rstats::Size i = 0; i < Rstats::Func::get_length(sv_r, sv_x1); i++) {
        if (v1->exists_na_position(i)) {
          v2->set_value<Rstats::Integer>(i, 1);
        }
        else {
    ...
    SV* is_factor(SV* sv_r, SV* sv_x1) {
      ...
      Rstats::Size v_classes_length = Rstats::Func::get_length(sv_r, sv_classes);
      
      Rstats::Logical match = 0;
      for (Rstats::Size i = 0; i < v_classes_length; i++) {
    ...
    SV* compose(SV* sv_r, SV* sv_type, SV* sv_elements)
    {
      ...
      for (Rstats::Size i = 0; i < na_positions.size(); i++) {
    ...
    SV* args_h(SV* sv_r, SV* sv_names, SV* sv_args) {
      ...
      Rstats::Size args_length = Rstats::pl_av_len(sv_args);
      SV* sv_opt;
      ...
      Rstats::Size names_length = Rstats::pl_av_len(sv_names);
      for (Rstats::Size i = 0; i < names_length; i++) {
