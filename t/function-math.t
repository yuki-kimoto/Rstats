...
# abs
{
  ...
  {
    my $x1 = r->abs($r->c_(-3, 4));
    is_deeply($x1->values, [3, 4]);
  }

  # abs - matrix
  {
    my $x1 = r->abs($r->matrix($r->c_(-3, 4)));
    is_deeply($x1->values, [3, 4]);
  }
  
  # abs - complex
  {
    my $x1 = $r->c_(3 + 4*$r->i, 6 + 8*$r->i);
    my $x2 = r->abs($x1);
...
# atanh
{
  ...
  # atanh - double,array
  {
    my $x1 = $r->array($r->c_(0, 0.5, 1, 2, -1, -0.5, -2));
    my $x2 = r->atanh($x1);
    is($x2->values->[0], 0);
...
# acosh
{
  ...
  # acosh - integer
  {
    my $x1 = r->as->integer($r->array($r->c_(0, 1, 2)));
    my $x2 = r->acosh($x1);
    is($x2->values->[0], 'NaN');
    is($x2->values->[1], 0);
    is(sprintf("%.6f", $x2->values->[2]), '1.316958');
...
# asinh
{
  ...
  # asinh - integer
  {
    my $x1 = r->as->integer($r->array($r->c_(0, 1)));
    my $x2 = r->asinh($x1);
    is($x2->values->[0], '0');
    is(sprintf("%.6f", $x2->values->[1]), '0.881374');
    is_deeply(r->dim($x2)->values, [2]);
...
# cosh
{
  ...
  # cosh - double,array
  {
    my $x1 = $r->array($r->c_(0, $r->Inf, 2, -$r->Inf));
    my $x2 = r->cosh($x1);
    is($x2->values->[0], '1');
    is($x2->values->[1], 'Inf');
...
# cos
{
  ...
  # cos - double,array
  {
    my $x1 = $r->array($r->c_($r->pi/2, $r->pi/3));
    my $x2 = r->cos($x1);
...
# sin
{
  ...
  # sin - double,array
  {
    my $x1 = $r->array($r->c_($r->pi/2, $r->pi/6));
    my $x2 = r->sin($x1);
